...
class GoBackN:
  ...
  def __init__(self, local_port, remote_port, msg_handler):
    ...
    self.expected_sequence_number = 0 # Not in Selective Repeat. 
    self.receiver_last_ack = b'' # Not in Selective Repeat. 
  ...
  def handle_arrival_msg(self):
    ...
    # if receiver && msg_data.is_corrupt: send last ack
    # if receiver && expected_seq_num == 0: do nothing
    # if sender && msg_data.is_corrupt: do nothing
    if(msg_data.is_corrupt):
      if(self.is_receiver):
        if self.expected_sequence_number == 0: # if receiver: Not applicable to Selective Repeat
          util.log("Packet received is corrupted. " + self.NO_PREV_ACK_MSG) 
          return
        self.network_layer.send(self.receiver_last_ack) # if sender: Send previous ack
    ...
    if msg_data.msg_type == config.MSG_TYPE_ACK:
      ...
      if(self.sender_base == self.next_sequence_number):
        ...
        if self.timer.is_alive(): self.timer.cancel()
        self.set_timer() # Not applicable to Selective Repeat
        self.timer.start() # Not applicable to Selective Repeat
      self.sender_lock.release()
    ...
    # If DATA message, assume its for receiver
      ...
      if msg_data.seq_num == self.expected_sequence_number:
        ...
        self.receiver_last_ack = ack_pkt # Not applicable to Selective Repeat
        self.expected_sequence_number += 1 # Not applicable to Selective Repeat
  ...
  # Cleanup resources.
  def shutdown(self):
    if not self.is_receiver: self._wait_for_last_ACK() # Sender to wait for last ACK
    if self.timer.is_alive(): self.timer.cancel()
